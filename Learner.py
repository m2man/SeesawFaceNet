...
class face_learner(object):
    def __init__(self, conf, inference=False, pretrained=True, transfer=False):
        print(conf)
        if conf.seesawFaceNet:
            self.model = DW_seesawFaceNetv2(conf.embedding_size).to(conf.device)
...
#             self.scheduler = optim.lr_scheduler.ReduceLROnPlateau(self.optimizer, patience=40, verbose=True)
    ...
    def load_state(self, conf, fixed_str, from_save_folder=False, model_only=False):
        if from_save_folder:
            ...
            save_path = conf.model_path        
        model_name = re.sub("\x08", "", 'model_{}'.format(fixed_str))
        head_name = re.sub("\x08", "", 'head_{}'.format(fixed_str))
        opt_name = re.sub("\x08", "", 'optimizer_{}'.format(fixed_str))
        ...
        if not model_only:
            new_state_dict = OrderedDict()
            state_dict = torch.load(save_path/head_name) 
            for k, v in state_dict.items():
                if 'module.' in k:
                    name = k[7:] # remove `module.`
                    new_state_dict[name] = v
            ...
            new_state_dict = OrderedDict()
            state_dict = torch.load(save_path/opt_name) 
            for k, v in state_dict.items():
                if 'module.' in k:
                    name = k[7:] # remove `module.`
                    new_state_dict[name] = v
...
#         self.writer.add_scalar('{}_far:False Acceptance Ratio'.format(db_name), far, self.step)
    ...
    def train(self, conf, epochs):
        if self.transfer:
            self.load_state(conf, 'DW-SeesawFaceNet_V2.pth', from_save_folder=True, model_only=True)
        self.model.train()
        running_loss = 0.
